...
async function checkPreData (dataList = []) {
    function sameProperty(obj1, obj2, propertys = []) {
        return propertys.every(key => obj1[key] === obj2[key])
    }

    try {
        const {default: preData} = await import('../data/vacation.js')
...
async function main () {
    ...
    await checkPreData(dayData)
    
    const ics = creator({
        calName: '节假日',
    }, dayData)
    
    fs.writeFile(path.resolve(process.cwd(), './data/vacation.js'), 'export default ' + JSON.stringify(dayData, null, 4), () => {})
    fs.writeFile(path.resolve(process.cwd(), './data/vacation.ics'), ics, () => {})
}
